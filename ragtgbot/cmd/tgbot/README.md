...
4. Проверить текущую привязку можно командой `/channel`.

Важно: автоматическое переключение работает при получении `/start payload`. Отправка deep-link в обычном тексте не переключает канал.
